...
6. 中文日志系统：
   ...
   - 日志文件路径：/app/bot/logs/YYYY-MM-DD.log
...
UNMATCHED_REPLY_COOLDOWN = 3600  # 1小时

# =========================
# 📝 日志配置（按天生成）
# =========================
log_file_path = LOG_DIR / f"{time.strftime('%Y-%m-%d')}.log"
...
def group_summary():
    ...
    返回当前分组状态，按 key 排序输出
    """
    return {key: len(GROUPS[key]) for key in sorted(GROUPS.keys())}
...
def extract_visible_tags(msg):
    ...
    # 修复 entities 为 None 的情况
    entities = getattr(msg, "entities", None) or []

    strikes = []
    for entity in entities:
        if isinstance(entity, MessageEntityStrike):
            strikes.append((entity.offset, entity.offset + entity.length))

    tags = []
    for match in TAG_PATTERN.finditer(text):
        start, end = match.span()
        # 如果标签在删除线范围内，则忽略
        if any(s <= start < e or s < end <= e for s, e in strikes):
            continue
...
async def send_media(client, chat_id, msg):
    try:
        ...
        if duration is not None:
            logging.info(
                f"⏱️ 视频动态TTL：视频时长={duration}秒，缓冲={VIDEO_TTL_BUFFER_SECONDS}秒，最终TTL={ttl_seconds}秒，消息ID：{msg.id}"
            )
        else:
        ...
        if msg.photo:
            ...
            return False
        logging.info(f"📤 开始发送{media_type}，消息ID：{msg.id}，大小：{len(file_bytes)} 字节，TTL={ttl_seconds}秒")
        await client.send_file(chat_id, bio, force_document=False, spoiler=True, ttl=ttl_seconds)
    ...
    async def send_one(msg):
        async with semaphore:
            try:
                ...
                logging.error(f"❌ 处理媒体异常：用户={user_log}，分组={key}，消息ID={getattr(msg, 'id', '未知')}，错误={e}")
                return False
    results = await asyncio.gather(*(send_one(msg) for msg in send_msgs), return_exceptions=True)
...
async def handle_private_message(client, event):
    ...
    if not key or key not in GROUPS:
        ...
        if now - last_time >= UNMATCHED_REPLY_COOLDOWN:
            await event.reply("❌未匹配到关键词（示例：J1 或 #J1 或 j1 或 #j1），1小时内不再重复发送此消息！！！")
...
async def main():
    ...
    async def edited_watcher(event):
        msg = event.message
        tags = extract_visible_tags(msg)
        if not tags:
            removed = False
            for key in list(GROUPS.keys()):
                if msg.id in GROUPS[key]:
                    GROUPS[key].remove(msg.id)
                    removed = True
                    logging.info(f"⚠️ 消息标签删除，已移除分组 {key}，消息ID {msg.id}")
            if removed:
                normalize_groups()
                save_cache()
                logging.info(f"📦 编辑同步后当前分组状态：{group_summary()}")
            return
        new_tag = tags[0].lower()
        changed = False
        for key in list(GROUPS.keys()):
            if msg.id in GROUPS[key] and key != new_tag:
                GROUPS[key].remove(msg.id)
                changed = True
                logging.info(f"⚠️ 消息标签修改，已移除原分组 {key}，消息ID {msg.id}")
        added = add_media_to_group(new_tag, msg.id)
        if added:
            changed = True
            logging.info(f"✅ 消息标签修改，已加入新分组 {new_tag}，消息ID {msg.id}")
        if changed:
            normalize_groups()
            save_cache()
            logging.info(f"📦 编辑同步后当前分组状态：{group_summary()}")
